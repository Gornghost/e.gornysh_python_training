...
def test_delete_random_contact_from_random_group(app, db, orm_db):
    if len(db.get_contact_list()) == 0:
        app.contact.create_contact(Contact(first_name="f_name", middle_name="m_name", last_name="l_name"))
    if len(db.get_group_list()) == 0:
        app.group.create(Group(name="TEST"))
    group = random.choice(db.get_group_list())
    if len(orm_db.get_contacts_in_group(Group(id=group.id))) == 0:
    ...
    assert contact not in orm_db.get_contacts_in_group(Group(id=group.id))